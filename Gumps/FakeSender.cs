...
namespace Gumps
{
    internal static class FakeSender
    {
        ...
        public static void SendNewGump(NewAnimalFormGump gump)
        {
            GumpBuilder<StaticStringsHandler> builder = GumpBuilder.ForStaticStrings();

            try
            {
                gump.BuildForTest(ref builder);
            }
            finally
            {
                builder.Dispose();
