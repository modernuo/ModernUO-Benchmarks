...
public class BenchmarkGumpLibDeflate
{
    ...
    public void LibDeflate()
    {
        ...
        var destLength = _packetBuffer.Length;
        var result = Server.Compression.LibDeflate.Pack(_packetBuffer, ref destLength, layout);

        if(result != Server.Compression.ZlibError.Okay)
        {
            throw new Exception();
        }    
    }
    ...
    public void Zlib()
    {
        ...
        var destLength = _packetBuffer.Length;
        var result = System.IO.Compression.Zlib.Pack(_packetBuffer, ref destLength, layout, System.IO.Compression.ZlibQuality.Default);

        if (result != System.IO.Compression.ZlibError.Okay)
        {
            throw new Exception();
        }
    }
