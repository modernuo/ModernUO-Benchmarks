...
public static class BenchmarkFixture
{
    ...
    /// a stale TileDataHash. Single source of truth — no separate bake program.
    /// </summary>
    private static void EnsureBakedFiles()
    {
        ...
        var liveHash = StepCache.ComputeLiveTileDataHash();
        ...
        foreach (var mapId in mapsToBake)
        {
            ...
            if (StepCache.TryReadTileDataHashFromFile(path, out var existingHash) && existingHash == liveHash)
            {
                continue;
            }
